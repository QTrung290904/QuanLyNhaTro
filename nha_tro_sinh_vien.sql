...
create database nha_tro_sinh_vien;

use nha_tro_sinh_vien;
...
CREATE TABLE electricity_readings (
    ...
    room_id INT NOT NULL,                         -- Phòng sử dụng điện
    reading_month DATE NOT NULL,                  -- Tháng ghi điện (vd: 2025-07-01)
    previous_reading INT NOT NULL,                -- Chỉ số tháng trước
    current_reading INT NOT NULL,                 -- Chỉ số tháng này
    unit_price INT DEFAULT 4000,                  -- Đơn giá điện ( nghìn_đồng/kWh)
    total_amount INT GENERATED ALWAYS AS ((current_reading - previous_reading) * unit_price) STORED,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (room_id) REFERENCES rooms(room_id)
);

CREATE TABLE services (
    service_id INT PRIMARY KEY AUTO_INCREMENT,                      -- Mã dịch vụ (tự tăng)
    service_name VARCHAR(100) NOT NULL,                             -- Tên dịch vụ (VD: Internet, Giặt ủi)
    unit_price DECIMAL(10, 2) NOT NULL,                             -- Đơn giá cho mỗi đơn vị dịch vụ
    billing_cycle ENUM('monthly', 'one-time') NOT NULL,             -- Chu kỳ tính phí: hàng tháng hoặc một lần
    is_active BOOLEAN DEFAULT TRUE                                  -- Dịch vụ còn hoạt động hay không (TRUE/FALSE)
);

CREATE TABLE service_usage (
    usage_id INT PRIMARY KEY AUTO_INCREMENT,                        -- Mã ghi nhận sử dụng dịch vụ (tự tăng)
    contract_id INT NOT NULL,                                       -- Mã hợp đồng thuê sử dụng dịch vụ
    service_id INT NOT NULL,                                        -- Mã dịch vụ được sử dụng
    quantity INT DEFAULT 1,                                         -- Số lượng dịch vụ được sử dụng (VD: số lần giặt)
    usage_month DATE,                                               -- Tháng sử dụng dịch vụ (vd: 2025-07-01)
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,                  -- Ngày ghi nhận sử dụng dịch vụ

    FOREIGN KEY (contract_id) REFERENCES contracts(contract_id),    -- Liên kết tới hợp đồng
    FOREIGN KEY (service_id) REFERENCES services(service_id)        -- Liên kết tới bảng dịch vụ
);
...
INSERT INTO electricity_readings (room_id, reading_month, previous_reading, current_reading, unit_price, created_at)
VALUES
(1, '2025-06-01', 1200, 1320, 4000, '2025-06-30 08:30:00'),
(2, '2025-07-01', 1320, 1470, 4000, '2025-07-31 08:35:00'),
(3, '2025-07-01', 1500, 1625, 4000, '2025-07-31 09:00:00'),
(4, '2025-07-01', 1100, 1220, 4000, '2025-07-30 10:00:00'),
(5, '2025-07-01', 1400, 1540, 4000, '2025-07-29 08:15:00'),
(6, '2025-07-01', 1600, 1755, 4000, '2025-07-28 07:50:00'),
(7, '2025-07-01', 1300, 1410, 4000, '2025-07-27 09:30:00'),
(8, '2025-07-01', 1250, 1390, 4000, '2025-07-26 11:10:00'),
(9, '2025-07-01', 1000, 1150, 4000, '2025-07-25 08:45:00'),
(10, '2025-07-01', 1700, 1875, 4000, '2025-07-24 10:30:00');

INSERT INTO services (service_name, unit_price, billing_cycle, is_active)
VALUES 
  ('Internet', 50000, 'monthly', TRUE), -- hàng tháng phải trả 
  ('Giặt ủi', 25000, 'one-time', TRUE), -- trả từng lần sử dụng  
  ('Gửi xe máy', 50000, 'monthly', TRUE),
  ('Dọn phòng', 40000, 'one-time', TRUE);
